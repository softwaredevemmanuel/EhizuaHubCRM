...
const CourseCurriculum = () => {
    ...
    const contentCourse = courseDetails.find((item) => item.course == courseParam.trim());
    ...
    useEffect(() => {
        async function fetchCourseDetails() {
            try {
                setLoading(true)
                const response = await axios.get('http://localhost:5000/api/hub-tutor/hub-course-details', {
                    headers: {
                        course: tutorCourse
                    },
                });
                        ...
                        <div className='bg-slate-400 h-full rounded-lg pb-4'>
                            <div className='grid grid-cols-1 sm:grid-cols-3 gap-2'>
                                ...
                                <div className='w-full sm:col-span-2 px-4 sm:px-0 md:pr-2 text-[#134574]'>
                                    <p className=' font-bold text-center justify-center pt-2'>{contentCourse.course}</p>
                                    <p className=' pt-2 '>{contentCourse.description}</p>
                                    <p className=' pt-2 text-xs'>Duration: {contentCourse.durationInWeeks} Weeks</p>
                                    <p className=' pt-2 text-xs'>Course Fee: ₦ {contentCourse.price}</p>
                                </div>
                            </div>
                                        ...
                                        <div
                                            ...
                                            {mainTopicHovered == mainTopic && (
                                                <div className="pr-4 pt-2">
                                                    <Link to={`/edit-curriculum/${tutorCourse}/${mainTopic}`} className="bg-slate-400 text-slate-600 px-4 rounded-md" >
                                                        Edit
                                                    </Link>
                                                </div>
                                            )}
