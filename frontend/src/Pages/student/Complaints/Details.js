...
const NewFeedComplain = () => {
    ...
    return (
        <div className="overflow-y-scroll w-full h-full hide-bar ">
            {!user ? (
                <StudentLogin />
            ) : (
                <div className={`relative z-0 w-full bg-[#C8D1DA] px-5 flex flex-col gap-3 ${reportTypeParam == 'feedback' ? 'h-screen' : 'h-full'} `}>
